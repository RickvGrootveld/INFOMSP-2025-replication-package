...
class CouplingMap:
    ...
    def __init__(self, couplinglist=None):
        ...
        """
        # Initialize an empty directed graph to represent qubit connections
        self.graph = nx.DiGraph()
        # Cache for distance matrix to avoid recomputation
        self._dist_matrix = None
        # Cache for sorted qubit list
        self._qubit_list = None

        # Add all edges from the coupling list if provided
    ...
    def add_physical_qubit(self, physical_qubit):
        ...
        """
        # Validate that the qubit identifier is an integer
        if not isinstance(physical_qubit, int):
            raise CouplingError("Physical qubits should be integers.")
        # Check if qubit already exists to prevent duplicates
        ...
        # Invalidate cached distance matrix and qubit list
        self._dist_matrix = None  
        self._qubit_list = None 
    ...
    def add_edge(self, src, dst):
        ...
        """
        # Ensure source qubit exists in the graph
        if src not in self.physical_qubits:
            self.add_physical_qubit(src)
        # Ensure destination qubit exists in the graph
    ...
    def subgraph(self, nodelist):
        ...
        """
        # Create a new empty CouplingMap
        subcoupling = CouplingMap()
        # Extract subgraph from the main graph
        subcoupling.graph = self.graph.subgraph(nodelist)
        # Ensure all nodes in the list are present in the subgraph
    ...
    def physical_qubits(self):
        ...
        """
        # Compute and cache the sorted qubit list if not already cached
    ...
    def is_connected(self):
        ...
        try:
            ...
            # Return False if graph is empty or invalid
            return False
    ...
    def _compute_distance_matrix(self):
        ...
        """
        # Ensure the graph is connected before computing distances
    ...
    def __str__(self):
        ...
        """
        string = ""
        # Format edges as a list of pairs if edges exist
        if self.get_edges():
            string += "["
