...
class Game2048(Environment):
    ...
    def available_actions(self):
        ...
        Returns:
            A list of action indices that are valid in the current state.
        """
        return [action for action in range(4) if self.is_action_available(action)]

    def is_action_available(self, action):
        """Checks if the given action is available in the current state.
        ...
        Returns:
            True if the action would change the board state, False otherwise.
        """
    ...
    def _is_action_available_left(self, state):
        ...
        This is an internal helper method that checks if tiles can be moved or merged
        when sliding left.
        
        Arguments:
            state:
                The state to check (may be rotated).
    ...
    def do_action(self, action):
        """Performs the given action and updates the game state.
        ...
        Returns:
            The reward obtained from this action (sum of merged tile values).
        """
        temp_state = np.rot90(self._state, action)
    ...
    def _do_action_left(self, state):
        """Performs a left action on the given state.
        
        This is an internal helper method that slides all tiles left and merges
        adjacent tiles with the same value.
        
        Arguments:
            state:
                The state to modify (may be rotated).
    ...
    def add_random_tile(self):
        ...
        The tile value is 2 (represented as 1) with 90% probability and
        4 (represented as 2) with 10% probability.
        """
