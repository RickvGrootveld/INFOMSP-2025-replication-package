...
class Game2048(Environment):
    ...
    def actions(self):
        ...
        Returns:
            dict: Dictionary describing the action space (4 discrete actions).
        """
    ...
    def __init__(self, state=None, initial_score=0):
        ...
        if state is None:
            # Create empty 4x4 grid and add two random starting tiles
    ...
    def game_over(self):
        ...
        Returns:
            bool: True if no actions are available, False otherwise.
        """
    ...
    def is_action_available(self, action):
        ...
        Returns:
            bool: True if the action would modify the board, False otherwise.
        """
    ...
    def _is_action_available_left(self, state):
        """
        Check if a left action is available on the given state.
        
        An action is available if any tile can move left or merge with an adjacent tile.
        ...
        for row in range(4):
            ...
            for col in range(4):
                ...
                if state[row, col] != 0 and has_empty:
                    return True
                # A tile can merge with the tile to its left if they're equal
    ...
    def do_action(self, action):
        ...
        """
        # Rotate board to treat all actions as left moves
        temp_state = np.rot90(self._state, action)
    ...
    def _do_action_left(self, state):
        """
        Execute a left move on the given state, merging tiles and shifting them left.
        
        Args:
            state (np.ndarray): The game state to modify (modified in place).
            
        Returns:
            float: The reward from merging tiles (sum of merged tile values).
        """
        reward = 0
    ...
    def add_random_tile(self):
        """
        Add a random tile (2 with 90% probability, 4 with 10% probability) to an empty cell.
        
        The tile is placed at a randomly chosen empty position on the board.
        """
        x_pos, y_pos = np.where(self._state == 0)
        assert len(x_pos) != 0
        empty_index = np.random.choice(len(x_pos))
        # Value is stored as exponent (1 for 2, 2 for 4)
    ...
    def print_state(self):
        """
        Print a visual representation of the current game board to the console.
        
        Displays the board as a 4x4 grid with tile values and borders.
        """
    ...
    def state(self):
        ...
        Returns:
            np.ndarray: The 4x4 game board with tile values stored as exponents.
        """
        return self._state
    ...
    def score(self):
        ...
        Returns:
            int: The cumulative score from all merged tiles.
        """
        return self._score
