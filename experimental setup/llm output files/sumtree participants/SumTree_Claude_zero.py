...
class SumTree(object):
    ...
    def move(self, external_index, new_priority):
        ...
        Args:
            external_index: The index in the leaf node range (0-based from first leaf).
    ...
    def _update_internal_nodes(self, index, delta):
        ...
        Args:
            index: The index of the node whose priority changed.
            delta: The change in priority value.
        """
    ...
    def _next_position_then_increment(self):
        ...
        """
        start = self._capacity - 1  # First leaf node index
    ...
    def _sample_with_priority(self, p):
        ...
        based on cumulative priority sums.
        ...
        while True:
            ...
            if p <= left_p:
                # Sample falls in left subtree
                parent = left
            else:
                # Sample falls in right subtree
                if left + 1 >= len(self._memory):
                    raise RuntimeError('Right child is expected to exist.')
                p -= left_p  # Adjust p for right subtree
                parent = left + 1
    ...
    def __getitem__(self, index):
        """Get an item by its external index (0-based from first leaf).
        
        Args:
            index: The external index of the item.
    ...
    def __getslice__(self, start, end):
        """Get a slice of items from the leaf nodes.
